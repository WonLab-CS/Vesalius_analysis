...
# set directories
#-----------------------------------------------------------------------------#
if (!dir.exists("/common/martinp4/benchmarking_out/PASTE/")) {
    dir.create("/common/martinp4/benchmarking_out/PASTE/")
}
output_data <- "/common/martinp4/benchmarking_out/PASTE/"
...
# territory plots
#-----------------------------------------------------------------------------#
file_name <- paste0(output_data,"PASTE_output_plots.pdf")
...
for (i in seq_along(query)) {
    ...
    geom_point(size = 0.5) +
    theme_bw() +
    labs(title = paste("Reference", gsub("_spatial","",ref_tag), "sample",local_ref),color = "Territory") +
    ...
    geom_point(size = 0.5) +
    theme_bw() +
    labs(title = paste("Query", gsub("_spatial","",ref_tag), "sample",query_tag),color = "Territory")+
    scale_color_manual(values = cols ) +
    guides(colour = guide_legend(
        override.aes = list(size =  5)))
...
# Accuracy table
#-----------------------------------------------------------------------------#
file_name <- paste0(output_data,"PASTE_mapping_accuracy.csv")
