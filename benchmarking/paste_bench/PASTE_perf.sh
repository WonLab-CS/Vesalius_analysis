...
for value in "${unique_values[@]}"; do
  ...
  python ${script_loc}PASTE_bench.py --slurm_id ${SLURM_ARRAY_TASK_ID} --type "computational_performance_${value}" --input $data_input --output $output &
  PID=$!
