...
def export_coord(adata, output_path, tag):
    ...
    export_adata = pd.concat([barcodes, spa, adata.obs['cell_labels']], axis = 1)
...
def main():
    ...
    output_path = '/common/martinp4/benchmarking_out/PASTE'
    ...
    pi12 = pst.pairwise_align(seed, query, numItermax = 5000, use_gpu = False) 
    slices, pis = [seed, query], [pi12]
    new_slices = pst.stack_slices_pairwise(slices, pis)
    new_query = new_slices[1]
    tag = f'PASTE_aligned_{re.sub(".csv","",files[0][0])}_{re.sub(".csv","",files[1][0])}.csv'
