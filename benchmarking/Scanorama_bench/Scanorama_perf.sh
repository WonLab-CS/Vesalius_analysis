#!/bin/bash
#SBATCH --job-name=Scanorama_perf
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for value in "${unique_values[@]}"; do
  ...
  python ${script_loc}Scanorama_bench.py --slurm_id ${SLURM_ARRAY_TASK_ID} --type "computational_performance_${value}" --input $data_input --output $output &
  PID=$!
