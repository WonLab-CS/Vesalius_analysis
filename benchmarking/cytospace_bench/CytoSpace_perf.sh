#!/bin/bash
#SBATCH --job-name=CytoSpace_perf
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cyto_input="/common/martinp4/benchmarking_out/CytoSpace/report/"
...
for value in "${unique_values[@]}"; do
  ...
  type="computational_performance_${value}"
  lab="noLab"
  tmp="${cyto_input}${SLURM_ARRAY_TASK_ID}_${lab}_${type}/"
  mkdir -p $tmp

  python ${script_loc}CytoSpace_bench.py \
    $SLURM_ARRAY_TASK_ID \
    $type \
    $tmp \
    $data_input
  ...
  python ${script_loc}CytoSpace_clean.py $tmp $output
  rm -rf "${tmp}"
